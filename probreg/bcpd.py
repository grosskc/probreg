...
class BayesianCoherentPointDrift():
    ...
    def registration(self, target, w=0.0,
        ...
        for _ in range(maxiter):
            ...
            inv_target = res.transformation.inverse().transform(target)
            res = self.maximization_step(target, inv_target, estep_res, res.sigma2)
...
class CombinedBCPD(BayesianCoherentPointDrift):
    ...
    def maximization_step(self, target, inv_target, estep_res, sigma2_p=None):
        return self._maximization_step(self._source, target, estep_res,
                                       sigma2_p, self._update_scale)

    @staticmethod
    def _maximization_step(source, target, inv_target, estep_res,
        ...
        v_hat = s2s2 * np.linalg(np.kron(sigma_mat, np.identity(dim))) * np.kron(p1, np.ones(dim)) * (inv_target - source)
        u_hat = source + v_hat
